...
def network_log_reader(network_log_path):
...
def network_log_searcher(network_raw_info, stext):

    for line in network_raw_info:
        if stext in line:
            return True
...
def information_merge(meta_info_list, stext):
    ...
    for meta in meta_info_list:
        network_log_path = DOWNLOAD_FOLDER + meta['hashed_id']
        network_raw_info = network_log_reader(network_log_path)

        if network_log_searcher(network_raw_info, stext):
...
def network_parser_main(csv_fn, stext):
    meta_info_list = file_to_meta_extractor(csv_fn)
    network_log_urls_list = [BROWSERSTACK_AUT_SESSION_LOG_URL + meta['hashed_id'] + "/networklogs" for meta in meta_info_list]
    download(network_log_urls_list, DOWNLOAD_FOLDER)
...
if __name__ == '__main__':
    if len(sys.argv) == 3:
        ...
        network_parser_main(csv_filename, search_text)
    else:
